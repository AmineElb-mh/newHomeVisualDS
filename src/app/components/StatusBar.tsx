// Generic iOS-style status bar (clock + signal/wifi/battery icons). This is
// OS chrome, not editorial brand content, so unlike Header/TabRow/BottomNav
// it's shared as-is across every brand rather than living per-brand.
const svgPaths = {
  p1af82040:
...
function Time({ dark }: { dark?: boolean }) {
  return (
    <div className="h-[54px] relative shrink-0 w-[134.065px]" data-name="Time">
      <p
        className={`[word-break:break-word] absolute font-[family-name:var(--font-family-system)] font-[590] inset-[33.96%_36.71%_25.3%_36.96%] leading-[22px] text-[length:var(--font-size-md)] text-center whitespace-nowrap ${
...
function Levels({ dark }: { dark?: boolean }) {
  ...
  return (
    <div className="h-[54px] relative shrink-0 w-[134.065px]" data-name="Levels">
      <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 134.065 54">
        <g id="Levels">
          <g id="Battery">
            <rect height="12" id="Border" opacity="0.35" rx="3.8" stroke={`var(--stroke-0, ${fill})`} width="24" x="78.5" y="23.5" />
            <path d={svgPaths.p1af82040} fill={`var(--fill-0, ${fill})`} id="Cap" opacity="0.4" />
            <rect fill={`var(--fill-0, ${fill})`} height="9" id="Capacity" rx="2.5" width="21" x="80" y="25" />
          </g>
          <path clipRule="evenodd" d={svgPaths.p397d7f00} fill={`var(--fill-0, ${fill})`} fillRule="evenodd" id="Wifi" />
          <path clipRule="evenodd" d={svgPaths.p37488800} fill={`var(--fill-0, ${fill})`} fillRule="evenodd" id="Cellular Connection" />
        </g>
      </svg>
    </div>
  );
...
export default function StatusBar({ dark }: { dark?: boolean }) {
  return (
    <div
      className={`content-stretch flex items-center justify-between relative w-full ${dark ? "bg-transparent" : "bg-white"}`}
